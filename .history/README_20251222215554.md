...
[![Play at iso-city.com](https://img.shields.io/badge/Play-iso--city.com-blue?style=for-the-badge)](https://iso-city.com), [![MIT License](https://img.shields.io/badge/License-MIT-green.svg?style=for-the-badge)](LICENSE), [![PRs Welcome](https://img.shields.io/badge/PRs-welcome-brightgreen.svg?style=for-the-badge)],(https://github.com/amilich/isometric-city/pulls),[![React 19](https://img.shields.io/badge/React-19-61DAFB?logo=react&logoColor=white)](https://react.dev),[![Next.js 16](https://img.shields.io/badge/Next.js-16-black?logo=next.js)](https://nextjs.org),[![TypeScript](https://img.shields.io/badge/TypeScript-5-3178C6?logo=typescript&logoColor=white)](https://typescriptlang.org)
...
### 🏗️ 60+ Building Types
Residential evolves from houses to high-rises. Commercial grows from shops to malls. Watch construction scaffolding rise, or buildings fall to abandonment.
...
### 🔥 Emergencies
Fires spread between buildings. Police fight crime. Fire trucks respond. Neglect your services at your peril.
...
Click "Load Example" on the landing page to explore a pre-built metropolis, or dive into the 9 example cities in `src/resources/`.
...
| System | What It Does |
|--------|--------------|
| **Simulation Loop** | Tick-based city updates (50–750ms). Calculates demand, evolves buildings, spreads fires. |
| **Rendering Engine** | Canvas-based isometric projection. Depth-sorted sprites. Dynamic day/night lighting. |
| **Traffic System** | Vehicles spawn, pathfind along roads, despawn. Trains follow rail networks. |
| **Pedestrian System** | Citizens walk between buildings. React to parks, transit, services. |
| **Economic Model** | Tax revenue, service costs, budget allocation, demand curves for RCI zones. |
| **Multi-Save** | UUID-based city saves. lz-string compression for URL sharing. |
...
│
├── lib/
│   ├── simulation.ts             # City simulation logic (1000+ lines)
│   ├── renderConfig.ts           # Sprite pack definitions
│   └── shareState.ts             # URL compression for sharing
│
├── resources/
...
**Pull requests are warmly welcome.** This project is intentionally open to evolution.
...
The architecture is ready. The door is open. What would *you* build?
...
### Pro Tips
- Connect roads to map edges to discover neighboring cities
- Place power plants first — buildings need electricity to grow
- Parks boost land value and happiness
- Watch the demand bars (R/C/I) to know what to zone next
- Fire stations have limited range — spread them out
...
- **↑ ↑ ↓ ↓ ← → ← → B A** — Retro bonus ($50,000)
- Type **`motherlode`** — Major treasury boost ($1,000,000)
- Type **`vinnie`** — Have a chat with an old friend
...
See **[skills/adding-asset-sheets.md](skills/adding-asset-sheets.md)** for a complete guide to adding new sprite sheets and building types.

**Quick checklist:**
1. Add sprite PNG to `/public/assets/`
2. Add `BuildingType` to `src/types/game.ts`
3. Add `Tool` entry with cost/description
4. Configure sprite in `src/lib/renderConfig.ts`
...
- Inspired by **SimCity 2000** and the golden age of city builders
- Built with [Next.js](https://nextjs.org), [React](https://react.dev), [Radix UI](https://radix-ui.com), and [Tailwind CSS](https://tailwindcss.com)
- Sprites crafted with care for the isometric aesthetic
